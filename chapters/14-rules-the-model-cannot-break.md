...
- **Fix before publishing, not just before print.** This chapter describes, at design level, a path from an agent run to a target-cluster administrator credential. It is the author's own demo platform, but either fix it first and describe the fix, or confirm you are comfortable publishing it as open.
